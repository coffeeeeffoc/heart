	    ...
	    Petal.prototype = {
	        draw: function () {
	            ...
	            if((drawType == 2) && (this.bloom.p.x < 870) && (this.bloom.p.x > 400)){	            	
            		ctx.strokeStyle = "rgba(255,255,255,0)";           	
	            }else if((drawType == 1) && (whiteFlg == true)){
	            	ctx.strokeStyle = "rgba(255,255,255,0)";
	            }else{
	    ...
	    Bloom.prototype = {
	        draw: function (drawType) {
	            ...
	            for (var i = 0; i < this.petals.length; i++) {
	                ...
	                if (drawType == 2) {//arrow
		                //控制花瓣的显示角度
		                if((p.startAngle >180)&&(p.startAngle < 300)){
		                	p.render();
	                	}
	                }else{
	                	p.render();
	    ...
	    Garden.randomrgba = function (rmin, rmax, gmin, gmax, bmin, bmax, a) {
			...
			if (Math.abs(r - g) <= limit && Math.abs(g - b) <= limit && Math.abs(b - r) <= limit) {
				return Garden.rgba(rmin, gmin, bmin, a);
			} else {
